...
def backprop(inputs,
    ...
    #init
    grad_bias[-1] = derivee(
        list_out[-1]) * (list_out[-1] - np.array(th_outputs))
    grad_weight[-1] = np.dot(list_out[-2][None].T, grad_bias[-1][None])
    #recurrence
    for col in range(len(list_out) - 3, -1, -1):
        grad_bias[col] = np.dot(
            weights[col + 1], grad_bias[col + 1]) * derivee(list_out[col + 1])
        grad_weight[col] = np.dot(list_out[col][None].T, grad_bias[col][None])
    return grad_weight, grad_bias, np.linalg.norm(th_outputs -
                                                  list_out[-1]) / 2


def random_w_b(inputs, reseau):
    weights = [
        2 * np.random.random((len(inputs), reseau[0])) - np.ones(
            (len(inputs), reseau[0]))
    ] + [
        2 * np.random.random((reseau[k], reseau[k + 1])) - np.ones(
            (reseau[k], reseau[k + 1])) for k in range(len(reseau) - 1)
    ]
    bias = [np.zeros(reseau[k]) for k in range(len(reseau))]
    return weights, bias


def cost_function(L_inputs,
                  L_th_outputs,
                  reseau,
                  weights,
                  bias,
...
def batch_training(L_inputs,
    ...
    for k in range(iterations):
        ...
        for data in range(len(L_inputs)):
            gw, gb, cost = backprop(L_inputs[data], L_th_outputs[data], reseau,
                                    weights, bias, activation, derivee)
...
def minibatch(L_inputs,
    ...
    for N in range(iterations):
        for minibatch in range(len(batchs_L_inputs)):
            batch_training(batchs_L_inputs[minibatch],
                           batchs_L_th_outputs[minibatch], reseau, weights,
                           bias, rate, 1, activation,
                           derivee)  #change weights et bias dans la fonction
            ...
            for data in range(len(L_inputs_test)):
                gw, gb, cost = backprop(L_inputs_test[data],
                                        L_th_outputs_test[data], reseau,
                                        weights, bias, activation, derivee)
...
def stochastic_training(total_inputs,
                        total_ouputs,
                        ini_weight,
                        ini_bias,
                        vitesse,
...
def le_xor_batch(pas, reseau=[4, 1], activation=sigmoid,
                 derivee=dsigmoid):  #pas=0.2 marche bien
    (Wt, Bt) = random_w_b([0, 0], reseau)
    N = 20000
    I = [
        np.array(
            [int(i % 4 == 0 or i % 4 == 1),
             int(i % 4 == 0 or i % 4 == 3)]) for i in range(4)
    ]
    O = [
        np.array([
            int((I[i][0] > 0 and I[i][1] <= 0)
                or (I[i][0] <= 0 and I[i][1] > 0))
        ]) for i in range(4)
    ]
    (nW, nB, E) = batch_training(I, O, reseau, Wt, Bt, pas, N, activation,
                                 derivee)
...
def le_xor_mini_batch(pas, reseau=[4, 1], activation=sigmoid,
                      derivee=dsigmoid):
    (Wt, Bt) = random_w_b([0, 0], reseau)
    N = 10000
    I = [
    ...
    (nW, nB, E) = minibatch(I, O, I, O, reseau, Wt, Bt, pas, N, 2, activation,
                            derivee)
    R11 = front_prop(I[0], reseau, nW, nB, activation)
    R10 = front_prop(I[1], reseau, nW, nB, activation)
    R00 = front_prop(I[2], reseau, nW, nB, activation)
...
def le_xor_stochastic(pas,
                      N=20000,
                      reseau=[4, 1],
                      activation=sigmoid,
                      derivee=dsigmoid):  #☺marche bien pour 0.2
    (Wt, Bt) = random_w_b([0, 0], reseau)
    I = [
        np.array(
            [int(i % 4 == 0 or i % 4 == 1),
    ...
    O = [
        np.array([
            int((I[i][0] > 0 and I[i][1] < 0) or (I[i][0] < 0 and I[i][1] > 0))
        ]) for i in range(N)
    ]
    (nW, nB, E) = stochastic_training(I, O, Wt, Bt, pas, reseau, 1, activation,
                                      derivee)
    R11 = front_prop(I[0], reseau, nW, nB, activation)
    R10 = front_prop(I[1], reseau, nW, nB, activation)
    R00 = front_prop(I[2], reseau, nW, nB, activation)
    ...
    with open(test_images_path, "rb") as test_images:
        with open(test_labels_path, "rb") as test_results:
            ...
            test_result = np.array(
                [[int(i == test_result_input[j]) for i in range(10)]
...
def MNIST_stoch_training(Wi, Bi, reseau, iterations, start, stop, activation,
                         derivee):
    global train_input, train, result_input, result, test_input, test, test_result_input, test_result
    errors = []
    (W, B,
     E) = stochastic_training(train[start:stop], result[start:stop], Wi, Bi,
                              .1, reseau, iterations, activation, derivee)
...
def global_MNIST(points, activation=sigmoid, derivee=dsigmoid, iterations=3):
    ...
    indic = 0
    while it < iterations:
        (W, B, success, errors) = MNIST_stoch_training(
            W, B, reseau, 1, start, stop, activation, derivee)
...
def super_global_MNIST(points,
    ...
    list_success = []
    for i in range(nbr):
        (W, B, success_total, errors) = global_MNIST(points, activation,
                                                     derivee, iterations)
        list_success.append(success_total)
    return np.array(list_success)


def image(k=-1, result=-1
    ...
    res = np.array(
        [[[int((1 - train[k][j + 28 * i]) * 255) for ii in range(3)]
