...
def training(listXin,listYth,list_size_layers,weights,bias,step,iterations):
    for k in range(iterations):
        ...
        for data in range(len(listXin)):
            gw,gb,cost=backprop(listXin[data],listYth[data],list_size_layers,weights,bias)
...
weights=[np.array([[0.2,-0.5,0.4],[-0.4,0.6,0.1]]),np.array([[0.3],[-0.4],[0.2]])] #line is before, column is after
